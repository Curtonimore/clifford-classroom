...
async function handler(req: NextRequest) {
  console.log('Session handler called with method:', req.method);
  ...
  try {
    ...
    if (req.method === 'GET') {
      console.log('Calling GET handler for session');
      ...
      console.log('GET handler response status for session:', response.status);
      return response;
    } else if (req.method === 'POST') {
      console.log('Calling POST handler for session');
      ...
      console.log('POST handler response status for session:', response.status);
      return response;
    }
    
    // If the method is not GET or POST, return a 405 Method Not Allowed
    console.log('Method not allowed for session:', req.method);
