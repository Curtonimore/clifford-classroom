...
export default function Tools() {
  return (
    <main className="max-w-4xl mx-auto px-4 py-12 font-average">
      <h1 className="text-4xl font-bold text-darkgreen mb-4">Tools for Teaching</h1>
      ...
      <div>
        {tools.map((tool) => (
          <div key={tool.title} className="border p-4 rounded mb-4 hover:shadow transition bg-white flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-xl font-semibold text-darkgreen mb-1">{tool.title}</div>
              <div className="text-gray-700 mb-2 md:mb-0">{tool.description}</div>
            </div>
            <Link href={tool.href} className="bg-darkgreen text-white px-4 py-2 rounded hover:bg-green-900 font-semibold text-center md:ml-6 mt-2 md:mt-0 w-full md:w-auto">Preview Tool</Link>
          </div>
        ))}
      </div>
    </main>
